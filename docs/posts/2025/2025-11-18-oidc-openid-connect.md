...
sequenceDiagram
    ...
    rect rgb(255,200,200)
    Note over SPA: Tokens stored in JS memory/sessionStorage<br/>Accessible to XSS / browser extensions<br/>Main risk factor in SPA+PKCE
    end
    ...
    Note over SPA,DownstreamAPI: SPA uses ONLY access_token to call downstream API
    end
...
**OIDC Authorization Code Flow with BFF pattern and with refresh token for multiple Downstream APIs (API-1 and API-2):**
